...
struct SessionState {
    ...
    /// macOS: target app PID captured before the capsule/overlay can affect focus.
    /// Used by the OpenWhispr-style edit monitor after insertion.
...
async fn begin_session(inner: &Arc<Inner>) -> Result<(), String> {
    let current_session_id = {
        ...
        state.target_pid = crate::edit_monitor::capture_target_pid();
        ...
        if let Some(pid) = state.target_pid {
            log::info!("[coord] target_pid captured: {pid}");
        }
        state.session_id
    };
    #[cfg(target_os = "windows")]
...
async fn end_session(inner: &Arc<Inner>) -> Result<(), String> {
    ...
    let (focus_target, target_pid) = {
        let state = inner.state.lock();
        (state.focus_target, state.target_pid)
    };
    ...
    log::info!(
        "[coord] insertion completed status={status:?} target_pid={target_pid:?} focus_ready_for_paste={focus_ready_for_paste}"
    );

    if matches!(status, InsertStatus::Inserted | InsertStatus::PasteSent) {
        crate::edit_monitor::start_after_insert(
            inner.app.lock().clone(),
            crate::edit_monitor::EditMonitorSession {
                target_pid,
                original_text: raw.text.clone(),
                inserted_text: polished.clone(),
            },
        );
    }
...
async fn polish_or_passthrough(
    ...
    let vih_started = Instant::now();
    match crate::vih_bridge::rewrite(&raw.text).await {
        Ok(Some(result)) if result.route == "output_text" || result.route == "passthrough" => {
            log::info!(
                "[vih] rewrite ok route={} chars={} elapsed_ms={}",
                result.route,
                result.rewritten_text.chars().count(),
                vih_started.elapsed().as_millis()
            );
            return (result.rewritten_text, None);
        }
        Ok(Some(result)) => {
            log::warn!(
                "[vih] unsupported route={} from rewrite; falling back to OpenLess polish",
                result.route
            );
        }
        Ok(None) => {
            log::debug!("[vih] rewrite disabled or unavailable; using OpenLess polish");
        }
        Err(e) => {
            log::warn!("[vih] rewrite failed: {e}; falling back to OpenLess polish");
        }
    }
...
fn hide_capsule_window_if_present() {
    ...
    let title: Vec<u16> = "Open Typeless Capsule"
        .encode_utf16()
        .chain(once(0))
        .collect();
