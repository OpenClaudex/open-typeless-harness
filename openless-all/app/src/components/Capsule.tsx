...
function ProgressRail({ state, level }: ProgressRailProps) {
  const voice = Math.min(1, Math.max(0, level));
  const isProcessing = state === 'transcribing' || state === 'polishing';
  const tone = state === 'error' ? 'var(--ol-err)' : 'var(--ol-blue)';
  const width = state === 'done'
    ? '100%'
    : state === 'cancelled' || state === 'error'
      ? '58%'
      : `${Math.round(18 + voice * 82)}%`;
    ...
    >
      <span
        style={{
          display: 'block',
          height: '100%',
          width: isProcessing ? '38%' : width,
          borderRadius: 999,
          background: tone,
          opacity: state === 'cancelled' ? 0.45 : 0.9,
          transformOrigin: 'left center',
          transition: isProcessing ? undefined : 'width 0.12s var(--ol-motion-quick)',
          animation: isProcessing ? 'cap-progress 1.05s var(--ol-motion-soft) infinite' : undefined,
        }}
      />
    </div>
  );
    ...
    case 'polishing':
      center = (
        ...
        >
          ...
          >
            {state === 'transcribing' ? t('capsule.transcribing') : t('capsule.polishing')}
          </span>
        </div>
      );
      break;
    ...
    >
      ...
      <style>{`
        ...
        @keyframes cap-progress {
          0%   { transform: translateX(-120%) scaleX(.7); }
          55%  { transform: translateX(110%) scaleX(1.15); }
          100% { transform: translateX(285%) scaleX(.72); }
        }
      `}</style>
    </div>
  );
