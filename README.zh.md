...
<p align="center">
  • 🎙️ 聚焦输入框口述 • ✨ LLM 润色 • 🧠 本地 Speech Skills •
</p>
...
>
> **🛠️ 给构建者** -> 产品闭环是 `ASR transcript -> LLM polish -> focused-field insertion -> edit monitor -> local speech skills`。
...
- **🎙️ 聚焦输入框口述**：在你正在使用的 app 里直接说，不需要切到单独编辑器。
...
| 阶段 | 发生什么 | 为什么重要 |
| --- | --- | --- |
| 🎙️ Listen | 采集语音并生成 ASR 转写。 | 用户留在当前写作上下文里。 |
